...
<div class="container mx-auto p-4 max-w-md">
    ...
    <form action="{{ route('posts.store') }}" method="POST">
        ...
        <input type="text" name="title" id="title" value="{{ old('title') }}" class="w-full border rounded px-3 py-2 mb-4" required>

        <label for="content" class="block font-medium mb-1">Konten:</label>
        <textarea name="content" id="content" rows="5" class="w-full border rounded px-3 py-2 mb-4" required>{{ old('content') }}</textarea>
